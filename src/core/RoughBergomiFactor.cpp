...
inline void validate_common(std::span<const double> BH,
  ...
  if (BH.size() != m_paths * N) throw std::invalid_argument("BH size must be m_paths*N");
...
// Term-structure xi0(t)
void RoughBergomiFactor::compute(std::span<const double> BH,
                                 std::span<const double> time,
                                 std::size_t m_paths, std::size_t N,
                                 double H, std::span<const double> xi0t, double eta,
                                 std::span<double> Xi) const {
  validate_common(BH, time, m_paths, N, H, eta, Xi);

  // Accept xi0t size N (t1..tN) or N+1 (t0..tN)
